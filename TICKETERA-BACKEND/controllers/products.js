...
} = require("../databases/queries_products");
  ...
  try {
    getAllDBProducts()
      .then((result) => {
        logger.info(`<== getAllProducts`);
        loggerCSV.info(
          `getAllProducts, ${(new Date() - function_enter_time) / 1000}`
        );
        res.status(200).json({
          ok: true,
          value: result,
  ...
  try {
    getDBProduct(id)
      .then((result) => {
        ...
        loggerCSV.info(
          `getProduct, ${(new Date() - function_enter_time) / 1000}`
        );
        res.status(200).json({
          ok: true,
      ...
      .catch((error) => {
        logger.error(
          `getProduct => getProduct : params=> id=${id} error=> ${error}`
        );
      });
  } catch (error) {
...
const getProductsByBrandAndContract = async (req, res = response) => {
  ...
  let function_enter_time = new Date();
  logger.info(
    `getProductsByBrandAndContract. marca_id:${marca_id} contract: ${contract}`
  );
  try {
    getDBProductByBrandAndContract(marca_id, contract)
      .then((result) => {
        ...
        loggerCSV.info(
          `getProductsByBrandAndContract, ${
            (new Date() - function_enter_time) / 1000
          }`
        );
        res.status(200).json({
          ok: true,
      ...
      .catch((error) => {
        logger.error(
          `getProductsByBrandAndContract => getDBProductByBrandAndContract : params=> marca_id=> ${marca_id} contract=> ${contract} error=> ${error}`
        );
      });
  } catch (error) {
    logger.error(
      `getDBProductByBrandAndContract : params=> marca_id=> ${marca_id} contract=> ${contract} error=> ${error}`
    );
    res.status(500).json({
      ok: false,
      value: [],
  ...
  try {
    getDBProductByBrand(marca_id)
      .then((result) => {
        ...
        loggerCSV.info(
          `getDBProductByBrand, ${(new Date() - function_enter_time) / 1000}`
        );
        res.status(200).json({
          ok: true,
          value: result,
      ...
      .catch((error) => {
        logger.error(
          `getDBProductByBrand => getDBProductByBrand : params=> marca_id=> ${marca_id} error=> ${error}`
        );
      });
  } catch (error) {
    logger.error(
      `getDBProductByBrand : params=> marca_id=> ${marca_id} error=> ${error}`
    );
    res.status(500).json({
      ok: false,
      value: [],
...
const createProduct = async (req, res = response) => {
  ...
  logger.info(
    `createProduct nombre:${nombre} modelo:${modelo} habilitado:${habilitado} marca_id:${marca_id} `
  );
  ...
  try {
    createDBProduct(nombre, modelo, habilitado, marca_id)
      ...
      .catch((dataError) => {
        logger.error(
          `createProduct => createDBProduct : params=> nombre:${nombre} modelo:${modelo} habilitado:${habilitado} marca_id:${marca_id} error=> ${dataError}`
        );
        res.status(501).json({
          ok: false,
          error: dataError,
    ...
    logger.error(
      `createBrand => createDBProduct : params=> nombre:${nombre} modelo:${modelo} habilitado:${habilitado} marca_id:${marca_id} error=> ${error}`
    );
    res.status(500).json({
      ok: false,
      error: error,
...
const updateProduct = async (req, res = response) => {
  ...
  const { nombre, modelo, habilitado, marca_id } = req.body;
  logger.info(
    `updateProduct. id:${id} nombre:${nombre} modelo:${modelo} habilitado:${habilitado} marca_id:${marca_id}`
  );
  try {
      ...
      .then((result) => {
        if (result == 1) {
          res.status(200).json({
            ok: true,
            value: result,
            msg: `El producto '${nombre}' fue actualizado correctamente.`,
          });
        } else {
          return res.status(401).json({
            ok: false,
            msg: `El producto no pudo ser actualizado en el sistema.-`,
          });
        }
      })
      .catch((dataError) => {
        logger.error(
          `updateProduct  => updateDBProduct : params=> id=${id} nombre=${nombre}`
        );
        res.status(501).json({
          ok: false,
          error: dataError,
    ...
    logger.error(
      `updateProduct : params=> id=${id} nombre=${nombre} error=> ${error}`
    );
    res.status(500).json({
      ok: false,
      error: error,
...
const deleteProduct = async (req, res = response) => {
  ...
  try {
    ...
    deleteDBProduct(id)
      ...
      .catch((dataError) => {
        logger.error(
          `deleteProduct => deleteDBProduct: params=> id=${id} error=> ${dataError}`
        );
