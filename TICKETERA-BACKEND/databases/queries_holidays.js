...
const createDBHoliday = (fecha, descripcion) => {
    const return_promise = new Promise((resolve, reject) => {
        ...
        pooldata.getPool.query('SELECT public.f_ticketera_holiday_insert($1, $2);', [fecha, descripcion], (error, results) => {
            if (error) {
...
const deleteDBHoliday = (fecha) => {
    const return_promise = new Promise((resolve, reject) => {
        ...
        pooldata.getPool.query('select * from public.f_ticketera_holiday_delete($1);', [fecha], (error, results) => {
            if (error) {
                reject(error.message);
            }
            else {
