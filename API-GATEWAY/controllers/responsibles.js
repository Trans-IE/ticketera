...
const getAllResponsibles = async (req, res = response) => {
    ...
    logger.info(`==> getAllResponsibles - username:${username}`);
    let url = process.env.HOST_TICKETERA_BACKEND + "/entities/getAllResponsibles";
    ...
    try {
        ...
        if (resultado) {
            ...
            if (body.ok) {
                ...
                logger.error(`getAllResponsibles : ${body.msg}`);
                res.status(200).json({
                    ok: false,
                    msg: body.msg
                });
        ...
        logger.error(`getAllResponsibles : ${error.message}`);
        res.status(500).json({
            ok: false,
            error: error,
