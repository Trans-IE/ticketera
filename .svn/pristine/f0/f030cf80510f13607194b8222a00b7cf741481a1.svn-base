...
const updateCompany = async( req, res = response ) => {
    ...
    logger.info(`updateCompany. id:${id}  nombre:${nombre} direccion:${direccion} telefono:${telefono} mail:${mail}  `)
    try {
        ...
        .then( id_result => {
            ...
            if( id_result > 0 ) {
                ...
                return res.status(404).json({
                    ok: false,
                    msg: `La Empresa id ${id} no pudo ser encontrado en el sistema.`
                });
            }
            else {
                ...
                return res.status(401).json({
                    ok: false,
                    msg: 'La Empresa no pudo ser actualizado en el sistema.'
                });
            }
